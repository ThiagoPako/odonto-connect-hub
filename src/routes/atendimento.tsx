...
function ConsultaPage() {
  ...
  const [mostrarGravacao, setMostrarGravacao] = useState(false);
                      ...
                      {tabAtiva === "consulta" && (
                        <div className="space-y-4">
                          ...
                          <div className="border border-border/40 rounded-xl overflow-hidden">
                            ...
                            >
                              <span className="flex items-center gap-2">
                                <Mic className={`h-3.5 w-3.5 ${gravacoes.length > 0 ? "text-primary" : ""}`} />
                                Gravação de áudio
                                {gravacoes.length > 0 && (
                                  <span className="px-1.5 py-0.5 rounded bg-primary/10 text-primary text-[10px] font-medium">
                                    {gravacoes.length} gravação{gravacoes.length > 1 ? "ões" : ""}
                                  </span>
                                )}
                              </span>
                              <span className="flex items-center gap-2">
                                <kbd className="px-1.5 py-0.5 rounded bg-muted text-[10px] font-mono text-muted-foreground border border-border/60">R</kbd>
                                <ChevronRight className={`h-3.5 w-3.5 transition-transform ${mostrarGravacao ? "rotate-90" : ""}`} />
                              </span>
                            </button>
                              ...
                              <div className="px-4 pb-4 pt-1 border-t border-border/30 space-y-3">
                                ...
                                {gravacoes.length > 0 && (
                                  <div className="space-y-1.5">
                                    {gravacoes.map((g, idx) => (
                                      <div key={g.id} className="flex items-center gap-3 bg-muted/30 rounded-lg px-3 py-1.5">
                                        <Activity className="h-3 w-3 text-primary/60" />
                                        <span className="text-[11px] text-foreground">Gravação {idx + 1}</span>
                                        <span className="text-[11px] text-muted-foreground">{formatTime(g.duration)}</span>
                                    ...
                                    <button
                                      ...
                                      className="flex items-center gap-1.5 h-7 px-3 rounded-lg bg-primary/10 text-primary text-[11px] font-medium hover:bg-primary/20 transition-colors disabled:opacity-50"
                                    >
                          ...
                          <div className="flex justify-end pt-2">
                            <button
                              onClick={finalizarAtendimento}
                              className="flex items-center gap-2 h-9 px-5 rounded-lg bg-primary text-primary-foreground text-xs font-semibold hover:bg-primary/90 transition-all"
                            >
                              <Save className="h-3.5 w-3.5" /> Finalizar Atendimento
                            </button>
                          </div>
                        </div>
                      )}
