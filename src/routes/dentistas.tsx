...
function DentistasPage() {
  const [dentistas, setDentistas] = useState<Dentista[]>(mockDentistas);
  ...
  const handleSave = (data: Omit<Dentista, "id" | "criadoEm">) => {
    if (editingDentista) {
      setDentistas((prev) =>
        prev.map((d) =>
          d.id === editingDentista.id ? { ...d, ...data } : d
        )
      );
    } else {
      const newDentista: Dentista = {
        ...data,
        id: `d${Date.now()}`,
        criadoEm: new Date(),
      };
      ...
      <main className="flex-1 p-8 space-y-6 overflow-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 animate-fade-in">
          <div className="bg-card rounded-2xl border border-border/60 p-5 shadow-card">
            ...
            <p className="text-2xl font-bold text-card-foreground mt-1 font-heading">
              {dentistas.length}
            </p>
          </div>
          <div className="bg-card rounded-2xl border border-border/60 p-5 shadow-card">
          ...
          <div className="bg-card rounded-2xl border border-border/60 p-5 shadow-card">
            ...
            <p className="text-2xl font-bold text-destructive mt-1 font-heading">
              {dentistas.filter((d) => d.status === "inativo").length}
            </p>
          </div>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border/60">
                  <th className="text-left text-[11px] uppercase tracking-wider font-semibold text-muted-foreground px-6 py-4">
                    Dentista
                  </th>
                  <th className="text-left text-[11px] uppercase tracking-wider font-semibold text-muted-foreground px-6 py-4">
                    CRO
                  ...
                  <th className="text-right text-[11px] uppercase tracking-wider font-semibold text-muted-foreground px-6 py-4">
                    Ações
                  </th>
                </tr>
              </thead>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="relative inline-block">
                        <button
                          onClick={() =>
                            setMenuOpen(
                              menuOpen === dentista.id ? null : dentista.id
                            )
                          }
                        ...
                        {menuOpen === dentista.id && (
                          <div className="absolute right-0 top-full mt-1 w-44 bg-card rounded-xl border border-border/60 shadow-lg z-20 py-1">
                            <Link
                              to="/painel-dentista"
                              search={{ id: dentista.id }}
                              className="flex items-center gap-2 px-4 py-2.5 text-sm text-foreground hover:bg-muted/40 transition-colors w-full"
                            >
...
function DentistaFormModal({
  ...
  const [especialidade, setEspecialidade] = useState(
    dentista?.especialidade ?? especialidades[0]
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onSave({ nome, email, telefone, cro, especialidade, comissao, status });
  };
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="px-5 py-2.5 rounded-xl text-sm font-medium text-muted-foreground hover:bg-muted/60 transition-colors"
            >
              Cancelar
            </button>
            <button
